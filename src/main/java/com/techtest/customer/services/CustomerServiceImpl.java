...
public class CustomerServiceImpl implements CustomerService {
     ...
     */
    @Override
    public void createCustomer(Customer customer) throws CustomerCreationFailedException, CustomerAlreadyExistsException {
        ...
        ResponseEntity<Response> response = restTemplate
                    .exchange(endpointConfig.getCrmCreateCustomerEndpoint(), HttpMethod.POST, request, Response.class);

        if (response.getBody() != null &&
                Constants.CRM_RESPONSE_STATUS_SUCCESS.equals(response.getBody().getStatus())) {
            log.info("Customer created with status : {} with id : {}", response.getBody().getStatus(), customer.getCustomerId());
        } else {
            throw new CustomerCreationFailedException("Response received but content is null or unexpected.");
        }
    }
     ...
     */
    @Override
    public void updateCustomer(Customer customer) throws CustomerUpdateFailedException, CustomerCanNotBeFoundException {
        ...
        ResponseEntity<Response> response = restTemplate.exchange(endpointConfig.getCrmUpdateCustomerEndpoint(),
                HttpMethod.POST, request, Response.class);

        if (response.getBody() != null &&
                Constants.CRM_RESPONSE_STATUS_SUCCESS.equals(response.getBody().getStatus())) {
            log.info("Customer updated with status : {} with id : {}", response.getBody().getStatus(), customer.getCustomerId());
        } else {
            throw new CustomerUpdateFailedException("Response received with but content is null or unexpected.");
        }
     ...
     */
    @Override
    public void deleteCustomerById(String customerId) throws CustomerDeleteFailedException, CustomerCanNotBeFoundException {
        ...
        ResponseEntity<Response> response = restTemplate.exchange(endpointConfig.getCrmDeleteCustomerEndpoint(),
                HttpMethod.POST, request, Response.class);

        if (response.getBody() != null &&
                Constants.CRM_RESPONSE_STATUS_SUCCESS.equals(response.getBody().getStatus())) {
            log.info("Customer updated with status : {} with id : {}", response.getBody().getStatus(), customerId);
        } else {
            throw new CustomerDeleteFailedException("Response received but content is null or unexpected.");
        }
     ...
     */
    @Override
    public Customer getCustomerById(String customerId) throws CustomerCanNotBeFoundException, CustomerReadFailedException {
        ...
        if (profileEntity.getBody() != null) {
            ...
            throw new CustomerReadFailedException("Response received but content is null or unexpected.");
        }
    }
}
